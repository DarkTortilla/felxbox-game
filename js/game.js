var game = {
  ...
  setHandlers: function () {
    ...
    $("#labelReset").on("click", function () {
      var warningReset =
        messages.warningReset[game.language] || messages.warningReset.es;
    ...
    $(window)
      ...
      .on("hashchange", function () {
        game.language = window.location.hash.substring(2) || "es";
        game.translate();
  ...
  loadLevel: function (level) {
    ...
    if (level.id == 6) {
      alert(
        "Felicidades, has superado los primeros 5 nivles, y con ello has ganado una beca del 25% en nuestro proximo bootcamp en marzo. Sin embargo los malechores aun siguen sueltos. ¿Deseas continuar?"
      );
    } else if (level.id == 11) {
      alert(
        "en el campo de flexbox nunca tantos le debieron tanto a tan pocos MR.CSS.\n ¡Felicidades has completado 10 niveles!"
      );
    }
    ...
    var instructions =
      level.instructions[game.language] || level.instructions.es;
  ...
  compare: function () {
    ...
    $(".frog").each(function () {
      ...
      position.left = Math.floor(position.left) + 30;
    ...
    tokens.forEach(function (token, i) {
      ...
      if (trimmedToken === pName && !filled) {
        ...
        var val = !keywords.includes(tokens[i + 1].trim())
          ? tokens[i + 1].trim()
          : ""; // TODO: Maybe prop value validiation required IDK
  ...
  cron: function cuentaRegresiva() {
    const divCuenta = document.getElementById("cuenta-regresiva");
    let segundos=600;
    const intervalo = setInterval(() => {
      divCuenta.textContent = segundos;
      segundos--;

      if (segundos < 0) {
        clearInterval(intervalo);
        divCuenta.textContent = "¡Tiempo terminado!";

        let password;
        do {
          password = prompt("Ingrese la contraseña:");
        } while (password !== "Finux"); 
        game.start();
